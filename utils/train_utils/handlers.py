...
class HandlersComponent:
    ...
    def attach_trainer_events(self, trainer, model, optimizer, scheduler, dirname, filename_prefix, save_interval,
        ...
        to_save, handler = self._prepare_checkpoint(trainer, model, optimizer, scheduler, dirname, filename_prefix,
                                                    n_saved, multi_gpu)

        if resume:
            self.resume_last_checkpoint(to_save, dirname, filename_prefix)
    ...
    def resume_last_checkpoint(self, to_save, dirname, filename_prefix):
        checkpoint_file = self._get_last_checkpoint_file(dirname, filename_prefix)
    ...
    def attach_eval_events(self, trainer, model, evaluator, eval_dataloader, writer,
                           prefix, score_function_name, score_is_loss, patience, dirname):
        ...
        trainer.add_event_handler(Events.EPOCH_COMPLETED,
                                  self.create_log_validation_results(evaluator, eval_dataloader, writer, prefix))
        ...
        if prefix == "Eval Val":
            ...
            self.add_early_stopping(trainer, evaluator, score_function, patience)

            self.add_best_save(trainer, evaluator, model, score_function, dirname, score_function_name)
